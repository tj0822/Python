...
#             'PRICE_066270(SK컴즈).csv',
#             'PRICE_181710(NHN엔터테인먼트)_encoding.csv']
fileList = ['PRICE_023590(다우기술).csv',
            'PRICE_034220(LG디스플레이).csv',
            'PRICE_035420(Naver).csv',
            'PRICE_035720(카카오).csv',
            'PRICE_036570(엔씨소프트).csv',
...
for fileName in fileList:
    ...
    for n in range(1, nMax+1):
        ...
        for d in range(-1, k, -1):
            ...
            print((endDate-startDate).days, '일 동안', downLimit, n, '주씩 매수하면', '수익 확률 :', p, '평균 수익률 :',  plusRate, '손실 확률 :', m, '평균 손실률 :', minusRate, '본전 확률 :', e)
    ...
    outputFileName = fileName.split('.')[0] + '_output.csv'
    with open(outputFileName, 'w') as csvfile:
        header = csv.DictWriter(csvfile, fieldnames=["매수기준", "매수단위", "수익확률", "평균 수익률", "손실 확률", "평균 손실률", "본전 확률"], delimiter=',')
        header.writeheader()
