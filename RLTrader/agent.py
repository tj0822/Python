...
class Agent:
    ...
    def __init__(self, environment, min_trading_unit=1, max_trading_unit=2, delayed_reward_threshold=0.05):
        ...
        self.portfolio_value = 0  # balance + num_stocks * (현재 주식 가격) = 총 자산
        self.base_portfolio_value = 0  # 직전 학습 시점의 PV(원금)
    ...
    def get_states(self):
        ...
        self.ratio_hold = 1 if int(self.portfolio_value / self.environment.get_price()) == 0 else (self.num_stocks / int(self.portfolio_value / self.environment.get_price()))
    ...
    def act(self, action, confidence):
        ...
        if action == Agent.ACTION_BUY:
            ...
            # 보유 현금이 모자랄 경우 보유 현금으로 가능한 만큼 매수
            if balance < 0:
            ...
            self.num_stocks += trading_unit  # 보우 주식 수 갱신            
